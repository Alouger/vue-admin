      ...
      <el-table :data="tableData" border style="width: 100%">
        ...
        >
          <template #default="{ row }">
            ...
            <el-button type="info" @click="onShowRoleClick(row)" v-permission="['distributeRole']">{{
              $t('msg.excel.showRole')
            }}</el-button>
            <el-button type="danger" @click="onRemoveClick(row)" v-permission="['removeUser']">{{
              $t('msg.excel.remove')
            }}</el-button>
          </template>
        </el-table-column>
...
const onRemoveClick = (row) => {
  ...
  ElMessageBox.confirm(
    ...
    .catch(() => {}) // 经查阅，使用ElMessageBox.confirm()必须添加错误捕获，否则报错
}
...
// 在 `dialog` 关闭时重置 `selectUserId`，保证每次打开重新获取用户角色数据
